...
async function fetchEnergyData() {
    const response = await fetch('data/energy_values.json');
    const data = await response.json();
    return data;
}

// Update the calculator to use the fetched data
async function updateCalculator() {
    const energyData = await fetchEnergyData();

    // Assuming energyData contains keys like co2PerLitrePetrol, co2PerKwhCurrentMix, etc.
    const co2PerLitrePetrol = energyData.co2PerLitrePetrol || 2.31; // Default value if not present
    const co2PerKwhCurrentMix = energyData.co2PerKwhCurrentMix || 0.5; // Default value if not present
...
// Initialize the calculator with updated data
updateCalculator();
